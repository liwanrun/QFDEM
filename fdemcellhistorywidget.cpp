...
#include "ui_fdemcellhistorywidget.h"
#include "history.h"

#include <QtVariantProperty>

FDEMCellHistoryWidget::FDEMCellHistoryWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FDEMCellHistoryWidget)
{
    ui->setupUi(this);
...
FDEMCellHistoryWidget::~FDEMCellHistoryWidget()
{
    delete this->manager;
    delete this->factory;
...
void FDEMCellHistoryWidget::InitPropertyBrowser()
{
    QStringList names;
    ...
    names.clear();
    names << "Sxx" << "Sxy" << "Syy" << "Seq" << "Smin" << "Smax";
...
void FDEMCellHistoryWidget::ConnectSignalSlots()
{
    connect(this->manager, &QtVariantPropertyManager::valueChanged,
            this, &FDEMCellHistoryWidget::slotTreeBrowserValueChanged);
}

void FDEMCellHistoryWidget::UpdatePropertyBrowser(CellHistory *hist)
{
    this->history = hist;
...
void FDEMCellHistoryWidget::UpdateTableWidgetColumn(const QVector<size_t> &ids, int col)
{
    ui->tableWidget->setRowCount(ids.size());
    for(int row = 0; row < ids.size(); row++)
    {
        QTableWidgetItem* item = new QTableWidgetItem(QString::number(ids[row]));
        item->setTextAlignment(Qt::AlignVCenter | Qt:: AlignRight);
        ui->tableWidget->setItem(row, col, item);
    }
}

void FDEMCellHistoryWidget::slotTreeBrowserValueChanged(QtProperty *property, const QVariant &value)
{
