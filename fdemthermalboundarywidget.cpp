...
#include "ui_fdemthermalboundarywidget.h"
#include "boundary.h"

#include <QMessageBox>
#include <QtVariantProperty>

FDEMThermalBoundaryWidget::FDEMThermalBoundaryWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FDEMThermalBoundaryWidget)
{
    ui->setupUi(this);
...
FDEMThermalBoundaryWidget::~FDEMThermalBoundaryWidget()
{
    delete this->factory;
    delete this->manager;
...
void FDEMThermalBoundaryWidget::InitPropertyBrowser()
{
...
void FDEMThermalBoundaryWidget::ConnectSignalSlots()
{
    connect(this->manager, &QtVariantPropertyManager::valueChanged,
            this, &FDEMThermalBoundaryWidget::slotThermalBrowserValueChanged);
}

void FDEMThermalBoundaryWidget::UpdatePropertyBrowser(ThermalBoundary *bnd)
{
    this->boundary = bnd;
...
void FDEMThermalBoundaryWidget::CreateBoundaryConditionProperty()
{
    ...
    names << "Temperature (Dirichlet)" << "Heat Flux (Neumann)" << "Convection (Robin)";
...
void FDEMThermalBoundaryWidget::CreateBoundarySpatialOptionProperty()
{
...
void FDEMThermalBoundaryWidget::CreateBoundaryTemporalOptionProperty()
{
    ...
    ui->timeBrowser->setBackgroundColor(item, QColor(255, 230, 191));
}

void FDEMThermalBoundaryWidget::SetPropertyVisibility(const QString &id, bool visible)
{
    QtVariantProperty *property;
...
void FDEMThermalBoundaryWidget::SetPropertyExpanded(const QString &id, bool expanded)
{
    QtVariantProperty *property;
    property = static_cast<QtVariantProperty*>(manager->qtProperty(id));
...
void FDEMThermalBoundaryWidget::UpdateBoundaryConditionProperty()
{
    QtVariantProperty *property;

    property = static_cast<QtVariantProperty*>(manager->qtProperty(QString("NodeSetId")));
    property->setValue(this->boundary->nodeSetId);
...
void FDEMThermalBoundaryWidget::UpdateBoundarySpatialOptionProperty()
{
    QtVariantProperty *property;
    ...
    disconnect(ui->spaceTableWidget, &QTableWidget::itemChanged,
               this, &FDEMThermalBoundaryWidget::slotSpatialTableWidgetItemChanged);
...
void FDEMThermalBoundaryWidget::UpdateBoundaryTemporalOptionProperty()
{
    QtVariantProperty *property;
    ...
    disconnect(ui->timeTableWidget, &QTableWidget::itemChanged,
               this, &FDEMThermalBoundaryWidget::slotTemporalTableWidgetItemChanged);
...
void FDEMThermalBoundaryWidget::slotThermalBrowserValueChanged(QtProperty *property, const QVariant &value)
{
...
void FDEMThermalBoundaryWidget::slotSpatialTableWidgetItemChanged(QTableWidgetItem *item)
{
    item->setTextAlignment(Qt::AlignVCenter | Qt:: AlignRight);
...
void FDEMThermalBoundaryWidget::slotTemporalTableWidgetItemChanged(QTableWidgetItem *item)
{
    item->setTextAlignment(Qt::AlignVCenter | Qt:: AlignRight);
    if(!item->data(Qt::EditRole).isNull())
    {
